    ...
    <div v-if="isLogin && cartList.length > 0">
      ...
      <div class="footer-fixed">
        ...
        <div class="all-total">
          ...
          <div v-if="!isEdit" class="goPay disabled" @click="goPay" :class="{ disabled: selCount === 0 }">GoPay({{ selCount }})
          </div>
          <div @click="handleDel" v-else :class="{ disabled: selCount === 0 }" class="delete disabled">删除</div>
        </div>
      </div>
    </div>
...
export default {
  ...
  methods: {
    ...
    toggleAllCheck () {
    // Click event triggers inversion
